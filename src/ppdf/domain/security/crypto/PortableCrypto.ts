/**
 * Browser-compatible Crypto Utility (Pure JS & Web Crypto API)
 * Implements MD5 and Random Bytes without depending on Node's 'crypto' or 'Buffer'
 * Ensures 100% portable execution in both browser and server environments.
 */

export class PortableCrypto {
  /**
  ...
  public static randomBytes(size: number): Uint8Array {
    ...
    // Fallback pseudo-random if crypto.getRandomValues is unavailable
    const arr = new Uint8Array(size);
  ...
  /**
   * Pure JS MD5 Implementation adhering to RFC 1321
   */
  public static md5(data: Uint8Array): Uint8Array {
    ...
    const wordsCount = (((msgLen + 8) >> 6) + 1) * 16;
    ...
    words[wordsCount - 2] = (msgLen * 8) & 0xffffffff;
    words[wordsCount - 1] = Math.floor((msgLen * 8) / 0x100000000);
    ...
    for (let i = 0; i < 4; i++) {
      out[i * 4 + 0] = resultWords[i] & 0xff;
      out[i * 4 + 1] = (resultWords[i] >> 8) & 0xff;
      out[i * 4 + 2] = (resultWords[i] >> 16) & 0xff;
      out[i * 4 + 3] = (resultWords[i] >> 24) & 0xff;
    }
    return out;
  }
}
