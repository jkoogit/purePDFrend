...
async function pushToBranch(branchName: string, commitMessage: string) {
  console.log(`\n▶ [Git Data API] '${branchName}' 브랜치 Push 시작...`);
...
async function main() {
  console.log('================================================================');
  console.log('🚀 [TASK-0013-01] 태스크 정리: 작업 브랜치 커밋 및 dev 동기화');
  console.log('================================================================');
  ...
  const commitMsg = 'docs: TASK-0013-01 docs/18.메뉴얼 신설 및 사용자/관리자 상세 매뉴얼(v1.0) 작성 & 대화턴 정렬 UI 보정';

  // 1. 작업 브랜치 Push
  const taskCommitSha = await pushToBranch(taskBranch, commitMsg);

  // 2. dev 브랜치 Push
  const devCommitSha = await pushToBranch('dev', commitMsg);

  console.log('\n================================================================');
  console.log('✨ [TASK-0013-01] 원격 Git Push 및 태스크 정리 완결');
...
main().catch((err) => {
  console.error('Task cleanup sync failed:', err);
  process.exit(1);
});
