...
2. **Web Worker 백그라운드 스레드 가동**: 메인 UI 스레드와 완전 격리된 백그라운드 Worker 스레드가 작동하며, `⚡ Web Worker Background` 상태 배지와 함께 페이지별 실시간 합성 진행률(0% ~ 100%)이 시각화됩니다.
3. **무프리징 & 컴파일 취소 지원**: 800쪽 대용량 도서 합성 중에도 메인 화면 스크롤이나 버튼 클릭이 멈추지(Freeze) 않으며, 필요 시 모달창의 **[컴파일 취소]** 버튼을 눌러 즉시 합성을 중단할 수 있습니다.
4. **0-Copy 고속 다운로드**: 합성이 완료되면 Transferable Object 기반 0-Copy 메모리 전송을 통해 표준 파일명 규격(`[도서제목]_ocr_YYYYMMDD.pdf`)으로 브라우저에 자동 다운로드됩니다.
5. 다운로드된 PDF는 Adobe Acrobat, Chrome, 브라우저 PDF 뷰어 등 어디서나 **텍스트 마우스 드래그, 복사(Ctrl+C), 전문 검색(Ctrl+F)**이 완벽하게 지원됩니다.
...
| 2026-09-25 | #16 | TASK-0013-02 | gemini | Web Worker 백그라운드 컴파일(무프리징/취소/0-Copy) 매뉴얼 현행화 (v1.1) | Gemini 3.7 Flash | Google Antigravity | [README_메뉴얼.md](./README_메뉴얼.md) |
