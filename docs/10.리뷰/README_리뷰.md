...
| `10-26` | `260924_026_800쪽_대용량_가상뷰어_메모리가드_및_로컬실행환경_보완_리뷰.md` | 800쪽 대용량 가상뷰어 메모리가드 및 로컬실행환경 보완 리뷰 | 800쪽 가상 윈도잉 뷰포트 엔진, 10페이지 LRU 메모리가드(38MB 방어), 로컬 .env 자동 로더, 긴급 Git Push 보안 필터링 및 30종 단위검증 완결 |
| `10-27` | `260924_027_PDF_페이지_레이아웃_편집기_및_무제한_Undo_Redo_구현_리뷰.md` | PDF 페이지 레이아웃 편집기 및 무제한 Undo/Redo 구현 리뷰 | 소프트삭제/클린징, Command/Memento 패턴 기반 무제한 히스토리 스택, 90°/180°/270° 회전 및 DnD 순서재배치, 단축키 연동 및 31종 단위검증 완결 |
| `10-28` | `260924_028_투명_텍스트_레이어_결합_Searchable_PDF_내보내기_구현_리뷰.md` | 투명 텍스트 레이어 결합 Searchable PDF 내보내기 구현 리뷰 | pdf-lib 투명 텍스트(opacity:0) 임베딩, 동적 폰트 레지스트리, [도서명]_ocr_YYYYMMDD.pdf 네이밍 및 원클릭 다운로드 파이프라인 완결 |
| `10-29` | `260924_029_In_Memory_Write_Through_설정관리자_및_2Way_BBox_교정기_구현_리뷰.md` | In-Memory 설정관리자 및 2-Way BBox 교정기 구현 리뷰 | 0ms Fast Read 캐시, Observer Pub-Sub, 3대 프리셋, 2-Way BBox 마우스 드래그/리사이즈/스냅, 500ms 디바운스 LiveSync 및 React 18 무한루프 방어 |
