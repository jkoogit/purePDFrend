...
| `10-24` | `260925_024_PDF_메타데이터_주입기_도서서지_활동_독서회독_도메인_구현_리뷰.md` | PDF 표준 및 커스텀 메타데이터 주입기 및 도서 서지·활동·독서 회독 도메인 구현 리뷰 | 표준 8대 메타데이터 + 12종 서지 + 4대 활동 + 독서 회독 히스토리(roundHistory) 3대 VO, UTF-16BE 무손실 인코딩, 파사드/팩토리 패턴 및 11대 단위테스트 통과 |
| `10-25` | `260926_025_PDF_암호화_및_상황별_보안전략_엔진_구현_리뷰.md` | PDF 암호화 및 상황별 보안전략 엔진(PdfSecurityFacade) 구현 및 서비스 전수 점검 거버넌스 리뷰 | 5대 상황별 보안전략, 4대 디자인 패턴, ISO 32000-1 키유도/비트마스크, 브라우저 호환 PortableCrypto 및 5단계 서비스 전수점검 간단/상세 리뷰 UI 완비 |
| `10-26` | `260926_026_PDF_보안_메타데이터_모달_연동_및_원격_실시간_크로스체크_리뷰.md` | PDF 보안/메타데이터 모달 연동 및 원격 실시간 크로스체크 리뷰 | 메타데이터/보안 듀얼탭 모달, PdfExportPipeline, WinAnsi 예외방어, 원격 실시간 크로스체크 API(승급완료/SHA일치 분리 피드백) 및 Fast-Forward 승급 체계 구축 |
| `10-27` | `260926_027_TDD_종합_회귀_점검_매뉴얼_및_AGENTS개정_리뷰.md` | TDD 종합 회귀 점검, 사용자 서비스 매뉴얼 및 AGENTS.md 승급규정 개정 코드리뷰 | TDD 24개 단위테스트 전수 회귀검증 100% 통과, 11-03 운영매뉴얼 발행, AGENTS.md [규칙 2.4 #태스크승급] 보고양식 표준화 및 무결성 100점 완결 |
